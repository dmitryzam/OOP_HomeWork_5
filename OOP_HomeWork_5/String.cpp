...
String::String(const char* str) {
	size_t counter{};	// счетчик кол-ва эдементов, расположенных в памяти по переданному местоположению строки
	while (*(str + counter++) != '\0');		// посчитаем кол-во элементов в памяти
	data = new char[counter];
	for (size_t i{}; i < counter; ++i) {
		data[i] = str[i];
	}
	size = counter - 1;  // -1 без учета '\0'
}
...
String::String(String&& other) : String() {		// делегируем КпУ
...
String& String::operator=(const char* str) {
	delete[] data;
	size_t counter{};	// счетчик кол-ва элементов, расположенных в памяти по переданному местоположению строки
	while (*(str + counter++) != '\0');		// посчитаем кол-во элементов в памяти
	data = new char[counter];
	for (size_t i{}; i < counter; ++i) {
		data[i] = str[i];
...
String& String::insert(size_t index, size_t count, char sym){
	...
	tmp.size = size + count;
	if(index != 0)	// заполнение tmp.data до index
		for (size_t i{}; i < index; ++i) {
			tmp.data[i] = this->data[i];			
		}
	for (size_t i{ index }, j = 0; j < count; ++i, ++j) {	// заполнение sym в tmp.data
		tmp.data[i] = sym;
	}
	if (index != size)	// заполнение tmp.data после index
...
String& String::insert(size_t index, const String& str){
	...
	for (size_t i{ index }, j = 0; j < str.size; ++i, ++j) {	// заполнение str в tmp.data
...
String& String::replace(size_t pos, size_t count, size_t count2, char sym){
	...
	if (pos != 0)	// заполнение tmp.data до pos
		for (size_t i{}; i < pos; ++i) {
			tmp.data[i] = this->data[i];
		}
	for (size_t i{ pos }, j = 0; j < count2; ++i, ++j) {	// заполнение sym в tmp.data
		tmp.data[i] = sym;
	}
	if (pos != size)	// заполнение tmp.data после pos
...
String& String::replace(size_t pos, size_t count, const String& str){
	...
	tmp.size = size + str.size;
	if (pos != 0)		// заполнение tmp.data до index
		for (size_t i{}; i < pos; ++i) {
			tmp.data[i] = this->data[i];
		}
	for (size_t i{ pos }, j = 0; j < str.size; ++i, ++j) {	// заполнение str в tmp.data
		tmp.data[i] = str.data[j];
	}
	if (pos != size)	// заполнение tmp.data после index
